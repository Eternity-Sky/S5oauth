    ...
    <div className="min-h-screen bg-gray-50 p-8 font-sans">
      <div className="max-w-4xl mx-auto">
        <div className="flex justify-between items-center mb-8">
          ...
          <a href="/test-client" className="text-blue-600 hover:underline">返回测试页</a>
        </div>
        ...
        {status === "error" && (
          <div className="p-8 bg-red-50 border border-red-100 rounded-2xl text-red-700">
            ...
            <p className="font-mono text-sm bg-white/50 p-4 rounded-lg">{error}</p>
          </div>
        )}
        ...
        {status === "success" && data && (
          <div className="space-y-6">
            <div className="bg-green-50 border border-green-100 p-6 rounded-2xl flex items-center gap-4">
              <div className="bg-green-500 text-white p-1 rounded-full text-xs">✓</div>
              <p className="text-green-800 font-bold">认证流程已成功完成！</p>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="bg-white p-6 rounded-2xl border shadow-sm">
                <h3 className="font-bold text-gray-700 mb-4">用户信息 (UserInfo)</h3>
                <div className="space-y-4">
                  <div className="flex items-center gap-4">
                    {data.user.picture && (
                      <img src={data.user.picture} className="w-12 h-12 rounded-full border" alt="Avatar" />
                    )}
                    <div>
              ...
              <div className="bg-white p-6 rounded-2xl border shadow-sm">
                <h3 className="font-bold text-gray-700 mb-4">令牌详情 (Tokens)</h3>
                <div className="space-y-4">
                  <div>
                    <p className="text-xs text-gray-400 uppercase font-bold mb-1 tracking-wider">Access Token</p>
                    <p className="font-mono text-[10px] break-all bg-gray-50 p-2 rounded border">
                      {data.tokens.access_token}
                    </p>
                  </div>
                  <div>
                    <p className="text-xs text-gray-400 uppercase font-bold mb-1 tracking-wider">ID Token</p>
                    <p className="font-mono text-[10px] break-all bg-gray-50 p-2 rounded border">
